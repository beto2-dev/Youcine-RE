...
## 我们的研究到此为止（2026-09-09）

特别感谢 **ChapzoMods** 一直以来的鼎力支持——r4-r8 迭代与 boot-test 证据体系是两个人共同的成果，这个里程碑同样属于他。

我们已经完成了最初的目标：**解开 APK 的壳，并让它至少能够运行（即使并不完全）**。`booteable-1.17.6` 构建在完全移除壳的情况下启动了真实的 YouCine UI（BOOT OK，零 FATAL 异常）；更深层的功能取决于上文记录的物化覆盖率，我们就把它留在这里。

如果有人想取用这份代码进行修改、扩展或继续研究，请遵守许可证（GNU GPL 3.0，见 `LICENSE` 与 [docs/en/07-legal.md](docs/en/07-legal.md)）的规则。
